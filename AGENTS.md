...
## コミット/PR ポリシー
- 1 task or checklist = 1 commit（厳守）。タスクを束ねない
- 完了時は同一コミットで `TODO.md` の当該項目を [x] に更新
...
- プロパティテスト（エージェント）：fast-check（`@fast-check/vitest`）で実施
  - 性質: 交点が両円を満たす、変換不変（回転・並進・一様スケール）、入力順対称
  - 既定: `vitest.setup.ts` にて `seed=424242`, `numRuns=200` を設定（実行時に `FC_SEED/FC_RUNS` で上書き可）
...
- 受け入れテストが全てGreen
- ユニット／プロパティテストがGreen（fast-check: シード固定または指定）
